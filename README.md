...
本仓库是 [ywddd/grok-inspection](https://github.com/ywddd/grok-inspection) 的 fork，上游版权归原作者（见 `LICENSE`）。自上游 v0.1.20 起的定制包括：

- v0.1.21：管理密钥只认管理中心官方 `cli-proxy-auth`、无密钥不发请求、开页单条鉴权校验、轮询自调度与单飞、鉴权失败（含本机临时封禁）立即熔断；详见 [管理密钥与临时封禁](#管理密钥与临时封禁)
- v0.1.22 / v0.1.23：插件元数据的仓库链接与作者字段指向本仓库（`Hkxtor`）
