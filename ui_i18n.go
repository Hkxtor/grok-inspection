...
const uiI18NJS = `  const I18N = {
    zh: {
      ...
      mgmt_key_invalid:'管理密钥不可用：请在管理中心重新登录并勾选“记住密码”。已停止轮询，避免把管理面板一起封禁。',
      mgmt_banned_prefix:'本机 IP 已被 CPA 临时封禁（管理鉴权连续失败）', mgmt_banned_about:'，约 ', mgmt_banned_minutes:' 分钟后自动解除',
      mgmt_banned_suffix:'；已停止轮询。立即恢复可重启 CPA（封禁记录只在内存里）。',
    ...
    en: {
      ...
      mgmt_key_invalid:'Management key unusable: log in at /management.html with remember-password. Polling stopped so the management panel is not banned along with it.',
      mgmt_banned_prefix:'This host IP is temporarily banned by CPA after repeated management auth failures', mgmt_banned_about:' (about ', mgmt_banned_minutes:' min left',
      mgmt_banned_suffix:'). Polling stopped. Restarting CPA clears the in-memory ban immediately.',
