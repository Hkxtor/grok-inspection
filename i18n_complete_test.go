...
func TestPluginVersionIs016(t *testing.T) {
	if pluginVersion != "0.1.21" {
		t.Fatalf("pluginVersion = %q, want 0.1.21", pluginVersion)
	}
	meta := pluginRegistration()
	if meta.Metadata.Version != "0.1.21" {
		t.Fatalf("meta.Version = %q", meta.Metadata.Version)
	}
	readme := mustReadRepoFile(t, "README.md")
	readmeEN := mustReadRepoFile(t, "README.en.md")
	if !strings.Contains(readme, "0.1.21") {
		t.Fatal("README.md missing 0.1.21")
	}
	if !strings.Contains(readmeEN, "0.1.21") {
		t.Fatal("README.en.md missing 0.1.21")
	}
